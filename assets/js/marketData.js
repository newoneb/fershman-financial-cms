...
function updateMarketTicker(marketData) {
    ...
    marketData.forEach(item => {
        ...
        tickerItem.innerHTML = `
            ...
            <span class="ticker-change ${item.direction}">${formatChangePercent(item.changePercent, item.direction)}</span>
        `;
...
/**
 * Format change percentage to avoid double plus signs
 * @param {string|number} changePercent Percentage change value
 * @param {string} direction Direction of change ('up' or 'down')
 * @returns {string} Properly formatted percentage change
 */
function formatChangePercent(changePercent, direction) {
    // Convert to string if it's a number
    let percentStr = changePercent.toString();
    
    // Remove any existing plus signs
    if (percentStr.startsWith('+')) {
        percentStr = percentStr.substring(1);
    }
    
    // Add plus sign only if it's positive and doesn't already have one
    if (direction === 'up') {
        return `+${percentStr}%`;
    } else {
        return `${percentStr}%`;
