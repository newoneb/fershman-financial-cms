...
function initScrollAnimations() {
    ...
    const headers = document.querySelectorAll('.page-header, .article-header, .hero-section');
    ...
    window.addEventListener('scroll', () => {
        ...
        headers.forEach(header => {
            if (scrollPosition <= 800) {
                ...
                const headerContent = header.querySelector('.page-header-content, .hero-content');
                if (headerContent) {
                    headerContent.style.opacity = opacity > 0 ? opacity : 0;
                    headerContent.style.transform = `translateY(${-translateY * 0.5}px)`;
                }
            }
        });
    });
